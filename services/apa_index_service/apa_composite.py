...
from src.utils import get_request_dt, get_lat_lon_from_tiff
...
def get_satellite_data(config: SHConfig, data_dir: str,
    ...
    for cropped_tif in cropped_tiffs:
        ...
        result_dict[date]['gps'] = get_lat_lon_from_tiff(cropped_tif)
...
def get_lake_box_boundaries(lake_query: str, crs='EPSG:4326') -> str:
    try:
        # Ensure lake_query is a string and not a list or tuple
        if isinstance(lake_query, (list, tuple)):
            lake_query = lake_query[0]
...
def get_dates_with_images(config: SHConfig, copernicus_data_service: str, bbox: BBox,
                          size: tuple, time_frame: list, resolution_in_m: int = 10,
                          max_cloud_coverage: float = 0.5) -> list:
    """
