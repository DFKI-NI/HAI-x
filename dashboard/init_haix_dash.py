...
import os
import ast
from utils import variables as var, dash_util as util, language_utils, evaluation_utils
        ...
        html.Div(id='page-content', children=[
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    dbc.Checklist(
                        [{"label": var_lang.AVOID, "value": var.AVOID},
                         {"label": var_lang.INTEREST, "value": var.INTEREST},
                         {"label": var_lang.TRAJECTORY, "value": var.TRAJECTORY},
                         {"label": var_lang.PATH, "value": var.PATH_PLANNING},
                         {"label": var_lang.EVALUATION, "value": var.EVALUATION}],
...
def init_callbacks(app):
    ...
    def update_graph(days_chosen, type_chosen):
        ...
        if var.TRAJECTORY in type_chosen:
            ...
            if len(days) != 0:
                ...
                for day_chosen in days:
                    dataname = str(day_chosen)

                    try:
                        lats = trajec["latitude"]
                        lons = trajec["longitude"]

                        if os.path.exists(var.VID_DATA_PATH + dataname + var.EXTRACTED_ROSBAGS_FOLDER_NAME + var.VIDEO_TIME_RGB_FILE_NAME):
                            has_video = True
                        else:
                            has_video = False
                        ...
                        customdataarray = np.concatenate((customdataarray, trajec['timestamp'].values.reshape(-1, 1)),
                                                         axis=1)
        ...
        if var.EVALUATION in type_chosen:
            days = tuple([d.strftime('%Y-%m-%d') for d in days_chosen])

            eval_data = db.open_table(var.SCHEMA, var.EVALUATION, var.EVAL_COLS, order_by='id')
            eval_data = eval_data[eval_data['date'].isin(days)]

            eval_data['weeding'] = eval_data['weeding'].astype(float)
            customdataarraynew = eval_data["weeding"].values.reshape(-1, 1)

            fig.add_scattermapbox(
                lat=eval_data.lat1,
                lon=eval_data.lon1,
                mode='markers',
                marker_color=eval_data["weeding"].values.tolist(),
                marker_colorscale='algae',
                customdata=customdataarraynew.astype(str),
                hovertemplate = 'Amount of Weed: %{customdata[0]}'            )
        ...
        if var.PATH_PLANNING in type_chosen:
            ...
            for x in path['path_id'].unique():
                ...
                for x in range(len(subpath) - 1):
                    ...
                    dist += geopy.distance.geodesic([subpath.iloc[x]['lat'], subpath.iloc[x]['lon']],
                                                    [subpath.iloc[x + 1]['lat'], subpath.iloc[x + 1]['lon']]).km
        ...
        if clickData is not None:
            if 'customdata' in clickData['points'][0].keys():
                ...
                if map_info[0] == var.seekuh:
                    ...
                    if informations != 0:
                        # Changed because we no longer display individual videos but the entire video in the interface.
                        # area_string = 'Area: (' + str(informations[1]) + ', ' + str(informations[2]) + ', ' + str(
                        #     informations[3]) + ', ' + str(informations[4]) + ')'
                        area_string = 'Position: (' + str(informations[1]) + ', ' + str(informations[2]) + ')'
                        ...
                        if child is not None:
                            ...
                            try:
                                ...
                                if video_container['id'] == 'video-container':
                                    block = child
                                    child['props']['children'][0]['props']['children'][2]['props'][
                                        'children'] = area_string
                            except:
                                block = None
                        ...
                        block = html.Div([html.P('CCoordinates: (' + str(lat) + ', ' + str(lon) + ')')])
                        data = {
                            'lat': lat,
                            'lon': lon
                        }
    ...
    def delete(area_n_clicks, point_n_clicks, path_n_clicks, storage):
        if area_n_clicks is None and point_n_clicks is None and path_n_clicks is None:
            ...
            return html.Div(
                '{} {} has been permanently deleted. Update the map or refresh the page to see the changes.'.format(
                    item, str(id)))
    ...
    def videoChanged(videoModeChosen, clickData, children):
        ...
        video_path_rgb = url_for('static',
                                 filename='video/' + map_info[1] + var.EXTRACTED_ROSBAGS_FOLDER_NAME + var.VIDEO_FILE_NAME_RGB)
        # videoname_ir = videoname.replace('.mp4', '_infra.mp4')
        # videoname_ir = "infra1_full.mp4"
        video_path_ir = url_for('static', filename='video/' + map_info[1] + var.EXTRACTED_ROSBAGS_FOLDER_NAME + var.VIDEO_FILE_NAME_IR)
