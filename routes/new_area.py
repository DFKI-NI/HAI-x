...
def add_single_new_area_to_db(req, var_lang):
    ...
    if len(cordpairs_list) > 0:
        cordpairs_list[0][0].append(cordpairs_list[0][0][0])

    number_polygons = len(cordpairs_list)

    if number_polygons < 1:
        return _render_template_helper(var_lang, aoi=False, failure_message="To create an area, please provide one polygon.")
...
def visualize_areas_of_interest(request_dict, var_lang, **kwargs):
    ...
    if not request_dict['day'] or not request_dict['lake_query']:
        ...
        return _render_template_helper(var_lang, aoi_result=aoi_result, aoi=True, submit_response=var_lang.NEW_AREA_SITE['input_data_missing'], available_dates=available_dates)

    request_str = 'http://estimate_areas_of_interest:10003/api/get_aois'

    data = requests.post(request_str, json=request_dict)
    logging.log(logging.INFO, f"Request: {data.status_code}")
    ...
    if data.status_code == 200:
        ...
        if len(apa_data.keys()) == 0:
            aoi_result = (f"For this date, no data is available \n\n "
                          ...
                          f"Output{apa_data}")
            return _render_template_helper(var_lang, aoi_result=aoi_result, aoi=True, available_dates=available_dates)
...
def get_possible_satellite_fly_overs(request_dict=None):
    ...
    if 'end' not in request_dict or not request_dict['end']:
        request_dict['end'] = datetime.today().strftime('%Y-%m-%d')
    if 'start' not in request_dict or not request_dict['start']:
        request_dict['start'] = (datetime.today() - timedelta(days=365)).strftime('%Y-%m-%d')

    request_str = 'http://estimate_areas_of_interest:10003/api/get_available_dates'

    try:
        dates = requests.post(request_str, json=request_dict, timeout=5)
        data = dates.json()
