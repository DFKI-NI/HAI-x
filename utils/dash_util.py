...
def get_index(own_timestamp: float, date: str):
    if os.path.exists(var.VID_DATA_PATH + date + var.EXTRACTED_ROSBAGS_FOLDER_NAME + var.VIDEO_TIME_RGB_FILE_NAME):
        data = pd.read_csv(var.VID_DATA_PATH + date + var.EXTRACTED_ROSBAGS_FOLDER_NAME + var.VIDEO_TIME_RGB_FILE_NAME)
        ...
        return [result_index, data.loc[result_index, 'Latitude'], data.loc[result_index, 'Longitude']]
    else:
        return 0
    
def get_time(own_timestamp: float, date: str, topic: str):
    if os.path.exists(var.VID_DATA_PATH + date + var.EXTRACTED_ROSBAGS_FOLDER_NAME + topic + "_full.csv"):
        data = pd.read_csv(var.VID_DATA_PATH + date + var.EXTRACTED_ROSBAGS_FOLDER_NAME + topic + "_full.csv")

        videoTime = data.iloc[-1]['VideoTime']
        for index, row in data.iterrows():
            if own_timestamp < row['Timestamp']:
...
def get_ir_time_by_rgb_time(rgb_time: float, date: str):
    if os.path.exists(var.VID_DATA_PATH + date + var.EXTRACTED_ROSBAGS_FOLDER_NAME + var.VIDEO_TIME_RGB_FILE_NAME) and \
        os.path.exists(var.VID_DATA_PATH + date + var.EXTRACTED_ROSBAGS_FOLDER_NAME + var.VIDEO_TIME_IR_FILE_NAME):
        data = pd.read_csv(var.VID_DATA_PATH + date + var.EXTRACTED_ROSBAGS_FOLDER_NAME + var.VIDEO_TIME_RGB_FILE_NAME)

        rgb_timestamp = data.iloc[-1]['Timestamp']
        for index, row in data.iterrows():
            if rgb_time <= row['VideoTime']:
                rgb_timestamp = row['Timestamp']
                break
        
        data = pd.read_csv(var.VID_DATA_PATH + date + var.EXTRACTED_ROSBAGS_FOLDER_NAME + var.VIDEO_TIME_IR_FILE_NAME)
        ir_time = data.iloc[-1]['VideoTime']
        for index, row in data.iterrows():
...
def append_type_to_dates(haix, pathplanning, traj, eval):
    ...
    dates = build_dates_dict(dates, eval, var.EVALUATION)
    date_choices = []
...
def add_boat_positions(fig, curentRgbTime, date, video_name):
    ...
    if os.path.exists(var.VID_DATA_PATH + date + var.EXTRACTED_ROSBAGS_FOLDER_NAME + video_name):
        data = pd.read_csv(var.VID_DATA_PATH + date + var.EXTRACTED_ROSBAGS_FOLDER_NAME + video_name)
